...
public class pageEntity extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		String pre = null;
		if(this.curpage == 1) {
			pre = "<a href=javascript:alert('已经是第一页');>下一页</a>";
		}else {
		...
		if(this.curpage == this.pages()) {
			next = "<a href=javascript:alert('已经是最后一页');>上一页</a>";
		}else {
		...
		select.append("<select onchange=\"location.href=this.options[this.selectedIndex].value;\" style=\"margin-right:10px;\">");
		for(int i=1; i<=this.pages(); i++){
		    if(i == this.curpage){
		         select.append("<option value='#' selected>").append(i).append("</option>");
		    }else{
		         select.append(String.format("<option value='%s?curpage=%d&pagesize=%d&%s'>%d</option>", this.controller, i, this.pagesize, this.where, i));
		    }
		}
		select.append("</select>");
		
		StringBuffer sb = new StringBuffer();
		sb.append("<div class='oy_p'>").append(first).append(pre).append(select).append(next).append(last).append(info).append("</div>");
		//输出
